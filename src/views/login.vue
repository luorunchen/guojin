  ...
  <div id="login">
      ...
      <el-row>
        ...
        <el-col :span="12">
          <div class="leftBox">
            ...
            <div class="tips">
              点击登录即同意 <el-link type="primary" @click="dialogVisible = true">《平台使用须知》</el-link>
            </div>
          </div>
        </el-col>
      </el-row>
    ...
    <el-dialog v-model="dialogVisible" title="平台协议" width="40%" center>
      <div class="agreement">
        ...
        <p>2.严禁用户在本平台发布涉黄、涉赌、反共、游戏等内容，否则应承担违规使用 的所有后果（含法律责任和经济赔偿责任）。</p>
        <p>3.平台提供的制度、职责、岗位操作规程、应急预案、风险告知牌等资料均为范 本，用户应根据自身特点予以修改、增减、补充、完善，并建立符合相关要求的 台账。</p>
        <p>4.用户法人及使用该平台的本单位其他人员误动作引起的资料消失、内容错误等，
          平台概不负责。</p>
      </div>
...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate((valid) => {
    if (valid) {
      ...
      login(numberValidateForm.userName, numberValidateForm.password, activeName.value).then(
        (res) => {
          if (res.data.code == 200) {
            ...
            sessionStorage.setItem('evaluation', res.data.data.evaluation_standard)
