    ...
    <el-table :data="gridData" height="450px">
      ...
      <el-table-column prop="address" label="操作">
        <template #default="scope">
          ...
          <el-switch v-model="value2[scope.$index]" class="mb-2" @change="switchChange(scope.$index, scope.row)"
            style="--el-switch-on-color: #13ce66; --el-switch-off-color: #ff4949" :loading="loading" />
...
const getUserInfoFun = (select: string) => {
  getUserInfo(formInline.user, formInline.region, pageSize3.value, currentPage3.value, props.type).then(res => {
    ...
    gridData.value.forEach((item, index) => {
      item.state == 1 ? value2.value[index] = false : value2.value[index] = true
    })
...
const switchChange = (index, row) => {
  ...
  ElMessageBox.confirm(
    `是否${!flag ? '禁用' : '解封'}<span style='color:red'>${row.username}</span>账号`,
    '提示',
    {
