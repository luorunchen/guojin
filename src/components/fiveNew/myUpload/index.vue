<template>
  ...
  <div class="rowBox" v-loading="loading" element-loading-text="正在加载中..." :element-loading-spinner="svg"
    ...
    <el-row :gutter="20">
      <el-col :span="4" v-for="(item, index) in getWxStoreListArray" :key="index" class="colBox">
        ...
        <div class="text">
          <el-tag effect="dark" :type="item.type == 1
              ? ''
              : item.type == 2
                ? 'success'
                : item.type == 3
  ...
  <el-dialog v-model="dialogVisible" title="我要上传" width="40%" center>
    <el-form label-width="auto" ref="formRef" :model="numberValidateForm" class="demo-ruleForm">
      ...
      <el-form-item label="所属分类" prop="type" :rules="[{ required: true, message: '请选择分类' }]">
        <el-select v-model="numberValidateForm.type" class="m-2">
          <el-option label="新技术" value="1" />
          <el-option label="新工艺" value="2" />
          <el-option label="新设备" value="3" />
          <el-option label="新材料" value="4" />
  ...
  <el-dialog v-model="detailsVisible" title="详情" width="40%" center>
    <div>
      <h6>浏览次数： {{ drawerRow.count }}</h6>
      <br />
      <h3>产品图片</h3>

      <div class="demo-image__preview">
        <el-image v-for="(info, index2) in props.activeName == '6'
            ? drawerRow.productImage.split(',')
            : drawerRow.productImage.split(',')" :key="index2" class="elImage" :src="info" :preview-src-list="props.activeName == '6'
        ? drawerRow.productImage.split(',')
        : drawerRow.productImage.split(',')
      ...
      <h3>产品优势</h3>
      <p class="info">
        {{ drawerRow.productSuper }}
      </p>
      <h3>产品介绍</h3>
      <p class="info">{{ drawerRow.productIntroduce }}</p>
      <h3>联系我们</h3>
      <p class="info">{{ drawerRow.phone }}</p>
    </div>
  </el-dialog>
</template>
...
const detailsFun = item => {
  ...
  drawerRow.value = item.content
  console.log(item, 'itemitem')
  addCount(item.id)
  // console.log(drawerRow.value.content, 'item')
}
