  ...
  <div class="info">
      ...
      <el-tab-pane label="其他信息" name="first">
        <el-form :inline="true" class="demo-form-inline" ref="ruleFormRef" :model="ruleForm" label-width="100px">
          <el-form-item label="上年产值" prop="name">
            <el-input v-model="ruleForm.last_value" placeholder="上年产值" disabled>
              <template #append> 万元 </template>
            </el-input>
          </el-form-item>
          <el-form-item label="员工人数">
            <el-input v-model="ruleForm.number" placeholder="请输入" disabled>
              <template #append> 人 </template>
            </el-input>
          </el-form-item>

          <el-form-item label="占地面积">
            <el-input v-model="ruleForm.floor_space" placeholder="请输入" disabled>
              <template #append> 平方 </template>
            </el-input>
          </el-form-item>
          <el-form-item label="经营面积">
            <el-input v-model="ruleForm.operation_space" placeholder="请输入" disabled>
              <template #append> 平方 </template>
            </el-input>
          </el-form-item>
          ...
          <el-form-item label="主要产品">
            <el-input v-model="ruleForm.products" placeholder="请输入" disabled />
          </el-form-item>
        </el-form>
      </el-tab-pane>
    </el-tabs>
    ...
    <el-tabs v-model="activeName" class="demo-tabs">
      <el-tab-pane label="相关人员联系方式" name="first">
        <el-form :inline="true" class="demo-form-inline" ref="ruleFormRef" :model="ruleForm" label-width="150px"
          label-position="left">
    ...
    <el-tabs v-model="activeName" class="demo-tabs">
      <el-tab-pane label="本单位涉及的场所" name="first">
        <el-form class="demo-form-inline" ref="ruleFormRef" :model="ruleForm" label-width="150px">
          <el-form-item :label="item.name" prop="resource" v-for="(item, index) in treeList" :key="index">
            <el-checkbox-group disabled v-model="ruleForm.resource" v-for="(item2, index2) in item.children"
              :key="index2">
              <el-checkbox :label="item2.id">{{ item2.name }}</el-checkbox>
            </el-checkbox-group>
          </el-form-item>
          <el-form-item label="(自行建立专用设备设施)" prop="resource" label-width="200px">
